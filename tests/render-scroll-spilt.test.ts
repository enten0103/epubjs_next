...
describe("createDrawer", () => {
  it("requires a configured book render prefix", async () => {
    ...
    await expect(drawer(root, "chapter-1.xhtml")).rejects.toThrow(
      "createDrawer requires book.resources.prefix",
    );
  });
  ...
  it("renders a same-sized iframe and redraws by navigating iframe src", async () => {
    ...
    const firstDraw = await drawer(root, "chapter-1.xhtml");
    const secondDraw = await drawer(root, "chapter-2.xhtml#chapter-2-target");
...
describe("scrollSpiltRender", () => {
  it("renders the first spine item into a fresh iframe", async () => {
    const root = createRoot();
  ...
  it("switches spine items by redrawing the iframe", async () => {
    const root = createRoot();
    const reader = scrollSpiltRender(TEST_PREFIX, root, TEST_BOOK);
    readers.push(reader);

    await reader.ready;
    const firstIframe = reader.iframe;

    await reader.controller.next();

    expect(reader.controller.getCurrent().html).toBe("chapter-2.xhtml");
    expect(reader.iframe).not.toBe(firstIframe);
